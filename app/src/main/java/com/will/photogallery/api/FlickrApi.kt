...
interface FlickrApi {
    ...
    // @GET @POST
    @GET("services/rest/?method=flickr.interestingness.getList") // 字符串 "/" 表示一个相对路径 URL，针对 baseURL 来说的相对路径
    ...
    // @Query 注解允许你动态拼接查询参数后再拼接到 URL 串里
    // 这里拼接的查询参数叫 text。text 的配对值是 query 变量
    // 比如 query 是 robot，那么产生的就是 text=robot
    // 拼接到 url 里
